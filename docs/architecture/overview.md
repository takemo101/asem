...
- Workspace-scoped Session tree and communication boundary
...
|---|---|---|---|
| `@asem/core` | pure domain / schema / port contracts | なし | Session / Message / Config schema、Workspace/location types、operation input/output、port interfaces、token hash、pure shell escaping helper |
...
| `@asem/store` | persistence adapter | SQLite | migrations、row mapping、Session / Message CRUD、Workspace-scoped transaction/query primitives、location filter primitives for worktree/repo views |
| `@asem/ops` | shared operation handlers | injected ports only | auth/Workspace checks、create/send/close/delete/list use-cases、operation-level cleanup semantics |
...
|---|---|---|
| Domain | Session / Message / Workspace boundary / location metadata の意味、schema、pure state rules | SQLite connection、shell execution、UI rendering |
...
| Operations | use-case orchestration、auth/Workspace checks、shared semantics、operation-level cleanup | surface-specific formatting、runtime-specific command strings、SQLite connection details |
...
逆に、Session / Message / Workspace boundary / location metadata の意味は project-wide に一つだけです。ここを surface や template ごとに再定義しない。
...
- Repo Alias resolution for `session create --repo <alias>` is shared operation behavior owned by `@asem/ops`, so CLI and MCP use the same cwd-alias semantics.
...
## Workspace boundary and location metadata
...
Within one Workspace, Session names are unique, parent-child relationships may cross cwd / Worktree Root boundaries, Messages may target any Session id in that Workspace, and `report parent` follows the current Session's stored `parent_session_id`.

`worktree_root` remains Session location metadata. It is used for runtime files, attach/close/delete execution context, TUI grouping, and CLI filters. It is not the normal parent/message/report boundary.

Existing `--scope worktree` UI may remain for compatibility, but the concept is a worktree filter over the Workspace Session tree.
