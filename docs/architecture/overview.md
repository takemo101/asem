...
| Mux Templates | herdr/tmux/zellij pane control steps | agent prompt semantics |
| Agent Templates | agent command and prompt delivery mode | multiplexer pane lifecycle |
...
- `store` / `runtime` / `ops` は `core` の schema / type / port contracts に依存してよい。
...
1. herdr / tmux / zellij のような multiplexer 差分を declarative command sequence に閉じ込める。
2. claude / codex / pi / agy / opencode の prompt delivery 差分を agent template に閉じ込める。
