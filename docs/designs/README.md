...
- [`asem-message-protocol-design.md`](./asem-message-protocol-design.md) — final Message protocol: valid authorized Messages persist before notification/template resolution and are retrieved by CLI/MCP pull; public Message envelope with `delivered` / `undelivered` / `failed` delivery states, cursor pagination, and bounded Inbox wait. Mux delivery is best-effort notification only.
