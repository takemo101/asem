...
The `asem run <agent> [--name <name>] [--prompt <text>] [--no-attach]` command (P0 follow-on of the [Message Protocol Design](./asem-message-protocol-design.md)) is a human-facing root-Agent launcher only, implemented as a thin CLI composition over the shared `create_session` operation with `root: true`. It requires the exact configured Agent Template name, defaults the Session name to that agent name, and hands the launched Agent a stable English CLI-local bootstrap prompt teaching the Message protocol, with the optional `--prompt` text appended under `## User request`. It starts a root Session, which may later become a parent by creating child Sessions. It has no `--parent` (including `--parent current`), no automatic parent detection, and no ambient-current fallback; child Agent launch stays `asem session create`, called by a registered parent Agent, so the detached child launcher is not duplicated. Its TTY attach behavior does not change this boundary — on a TTY it attaches after a successful create unless `--no-attach`, a non-TTY never attaches, and a failed attach preserves the running Session while exiting nonzero with the external attach exit status. `--no-attach` is a root-launch escape hatch, not a child-launch mode. Workspace / Worktree Root remains location metadata and never implies a parent Session.
