...
function makeSession(overrides: Partial<Session> = {}): Session {
  return {
    ...
    status: "running",
    tokenHash: hashToken("tok_current"),
...
describe("MCP tool calls", () => {
  test("list_sessions delegates to the same ops handler as direct operation calls", async () => {
    ...
    const direct = await listSessions(deps, {}, ctx);

    expect(mcp.isError).toBeUndefined();
    expect(parsed(mcp)).toEqual(direct.ok ? direct.value : direct.error);
  });

  test("get_session preserves attach_hint returned by ops", async () => {
    const store = new FakeStore();
    ...
    const direct = await getSession(deps, { id: "s_get" }, ctx);
