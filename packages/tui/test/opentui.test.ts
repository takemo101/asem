...
describe("session list rows", () => {
  ...
  test("renders Session rows inside a vertical-only scrollbox", () => {
  ...
  test("ellipsizes long Session rows without a horizontal scroll range", async () => {
    const rows: LeftRow[] = [
      {
        kind: "session",
        sessionId: "s1",
    ...
    const scrollboxElement = SessionRowsScrollBox({
      rows,
      bodyRows: 1,
    }) as ReactElement;
    const rowBox = reactChildren(reactProps(scrollboxElement).children).at(0);
    ...
    expect(scrollboxElement.props).toMatchObject({
      scrollY: true,
      scrollX: false,
    });
    ...
    const { renderer, mockMouse, renderOnce, captureCharFrame } =
      await testRender(scrollboxElement, { width: viewportWidth, height: 4 });
    await renderOnce();

    const scrollbox = findScrollBox(renderer.root);
    const renderedRow = renderer.root.findDescendantById("session-list-row:s1");
    if (scrollbox === null || renderedRow === undefined) {
      throw new Error("expected a rendered scrollbox and Session row");
    }

    const text = renderedRow.getChildren().at(0);
    if (!(text instanceof TextRenderable)) {
      throw new Error("expected a rendered Session row text element");
    }

    expect(scrollbox.scrollWidth).toBe(scrollbox.viewport.width);
    expect(renderedRow.width).toBe(scrollbox.viewport.width);
    expect(captureCharFrame()).toContain("...");

    await mockMouse.scroll(renderedRow.x + 1, renderedRow.y, "right");
